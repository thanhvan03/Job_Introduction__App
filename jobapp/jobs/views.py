...
from jobs.models import Category, JobListing, User, Comment, Rating, JobApplication, Tag
...
class CategoryViewSet(viewsets.ViewSet, generics.ListAPIView):
    ...
    def get_joblistings(self, request, pk):  # lấy danh sách các tin ứng tuyển của 1 ngành nghề (category) có id là pk
        ...
        joblistings = JobListing.objects.filter(category=category, active=True).all()

        return Response(serializers.JobListingSerializer(joblistings, many=True).data)


class BaseViewSet1(viewsets.ViewSet):
    queryset = JobListing.objects.filter(active=True).all()
    serializer_class = serializers.JobListingSerializer


class JobListingViewSet(BaseViewSet1, generics.ListAPIView):
    pagination_class = paginators.JobListingPaginator
    ...
    def search_job(self, request):
        ...
        return Response(serializers.JobListingSerializer(JobListing.objects.filter(
            (Q(title__icontains=keyword)) &
            Q(active=True)
        ), many=True).data)


class JobListingDetailViewSet(BaseViewSet1, generics.RetrieveAPIView):
    pass
